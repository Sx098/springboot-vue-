...
<div style="padding: 10px; width: 90%">
  ...
  <el-button type="primary" style="margin-left: 5px" @click="baidu1(search)">搜索
      ...
      <el-dropdown-menu slot="dropdown">
          ...
          <el-dropdown-item command="chato">按群聊</el-dropdown-item>
          </el-dropdown-menu>
        </el-dropdown>
    ...
    <el-table :data="tableData" border stripe style="width: 100%">
      <el-table-column prop="id" label="ID" sortable min-width="20%"></el-table-column>
        ...
        <template slot-scope="scope">
            <span style="display:block;font-size: large;text-align: center;">{{scope.row.sender}}</span>
            <img style="float: right;margin-right: 50px;" :src="scope.row.dizhifrom" width="50" height="50"/>
            </template>
        </el-table-column>
      <el-table-column prop="toname" label="群聊" min-width="25%">
        <template slot-scope="scope">
            <span style="display:block;font-size: large;text-align: center;">{{scope.row.fromgroup}}</span>
...
export default {
    ...
    methods:{
        loadTable(num) {
            ...
            axios.get(`api/public/page?pageNum=${this.currentPage}&pageSize=${this.pageSize}`)
            .then((res) => {
                this.tableData = res.data.data.records;
                for (let value of this.tableData) {
                    if(value.dizhito==null)
                    {value.dizhito="http://localhost:8888/9.jpg";}
                    if(value.dizhifrom==null)
                    {value.dizhifrom="http://localhost:8888/10.jpg";}
                    //获取头像地址，并添加到object对象中
                    axios.get(`api/people/cha/${value.sender}`)
                    .then((res)=>{
                         value.dizhifrom=`http://localhost:8888/${res.data.data[0].tou}.jpg`
                    })
                    .catch((err) => {console.error(err);});
                    axios.get(`api/qun/${value.fromgroup}`)
                    .then((res)=>{
                         value.dizhito=`http://localhost:8888/${res.data.data.tou}.jpg`
                         value.fromgroup=res.data.data.name
                    })
                    .catch((err) => {console.error(err);});
                    }
                    })
        ...
        handleCommand(command)
        {
            ...
            switch(command)
            {
                ...
                case "chato":this.sousuotypecn="请输入群聊"; break;
            }
        },
        baidu1(content){
          if(this.sousuotype=="chato"){content=1}
            axios.get(`api/public/${this.sousuotype}/${content}`)
            .then((res) => {
